...
Function CreateNPC.NPCs(NPCtype%, x#, y#, z#)
	...
	Select NPCtype
		...
		Case NPCtypeFGuard
			...
			;EntityRadius Collider, 0.15, 0.30
			EntityType n\Collider, HIT_PLAYER
			n\obj = CopyEntity(FGuardObj)
			
			n\Speed = (GetINIFloat("DATA\NPCs.ini", "Guard", "speed") / 100.0)
			temp# = (GetINIFloat("DATA\NPCs.ini", "Guard", "scale") / 2)
	...
	For n.NPCs = Each NPCs
		...
		Select n\NPCtype
			...
			Case NPCtypeZombie
				...
				;[End Block]
			Case NPCtypeGuard, NPCtypeFGuard ;------------------------------------------------------------------------------------------------------------------
				;[Block]
				prevFrame# = n\Frame
...
Function Console_SpawnNPC(c_input$, c_state$ = "")
	...
	Select c_input$ 
		...
		Case "female guard"
		    n.NPCs = CreateNPC(NPCtypeFGuard, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Female Guard spawned."
